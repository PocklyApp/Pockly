  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        ...
        await devLogin("jiaozongben@gmail.com", "James");
        if (cancelled) return;
        const browserState = await ensureBrowserDeviceState();
...
function PairPage({
  ...
  const install =
    tab === "unix"
      ? { label: "macOS / Linux", prompt: "$", command: "curl -fsSL https://cdn.pocklyapp.com/install.sh | bash" }
      : { label: "Windows PowerShell", prompt: "PS", command: "irm https://cdn.pocklyapp.com/install.ps1 | iex" };
  return (
