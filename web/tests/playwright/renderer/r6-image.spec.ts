...
// Image content + in-page lightbox.
...
test.describe("image lightbox", () => {
  ...
  test("renders a 300x300 thumbnail with data: URL src", async ({ page }) => {
  ...
  test("click opens the lightbox and locks body scroll", async ({ page }) => {
  ...
  test("ESC key closes the lightbox + restores body scroll", async ({ page }) => {
    await page.locator(".image-turn-card").first().click();
