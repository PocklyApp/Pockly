...
func TestEndpoint(t *testing.T) {
	...
	if got := Endpoint("https://pockly.example/"); got != "https://pockly.example/api/telemetry/daemon" {
		t.Fatalf("Endpoint() = %q", got)
	}
	t.Setenv("POCKLY_TELEMETRY_ENDPOINT", "http://127.0.0.1/telemetry")
	if got := Endpoint("https://pockly.example"); got != "http://127.0.0.1/telemetry" {
...
func TestSafeErrorCodeRedactsPathsAndSecrets(t *testing.T) {
	tests := map[string]string{
		...
		"Post \"https://pockly.example/api\": timeout": "timeout",
