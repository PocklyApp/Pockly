...
Install to a user-writable directory instead of `/usr/local/bin`:
    ...
    curl -fsSL https://cdn.pockly.example/install.sh | POCKLY_DAEMON_INSTALL_DIR="$HOME/.local/bin" bash

Install latest release on Windows PowerShell:

    irm https://cdn.pockly.example/install.ps1 | iex

Install a specific tag:

    curl -fsSL https://cdn.pockly.example/install.sh | POCKLY_DAEMON_VERSION=v0.1.0 bash
...
Telemetry goes to `https://pockly.example/api/telemetry/daemon` or the current
...
Override the relay proxy endpoint with:
    ...
    POCKLY_TELEMETRY_ENDPOINT=https://pockly.example/api/telemetry/daemon
