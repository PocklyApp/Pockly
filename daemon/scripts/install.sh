...
BASE_URL="${POCKLY_DAEMON_BASE_URL:-https://cdn.pockly.example/pockly-daemon}"
...
Fix — download the installer first, then run it with a real terminal:
  ...
  curl -fsSL https://cdn.pockly.example/install.sh -o /tmp/pockly-install.sh
  bash /tmp/pockly-install.sh

Alternative — pre-cache sudo credentials, then keep using the one-liner:

  sudo -v && curl -fsSL https://cdn.pockly.example/install.sh | bash

Or install into a directory you own (no sudo needed):

  curl -fsSL https://cdn.pockly.example/install.sh | \\
...
case "$(uname -s)" in
  ...
  *)
    ...
    echo "Use https://cdn.pockly.example/install.ps1 on Windows." >&2
    exit 1
    ;;
esac
...
if [ "${POCKLY_DAEMON_NO_SETUP:-}" != "1" ]; then
  relay_url="${POCKLY_RELAY_URL:-https://pockly.example}"
  echo
  ...
  echo "Run manually when ready: ${target} setup --relay-url ${POCKLY_RELAY_URL:-https://pockly.example}"
fi
